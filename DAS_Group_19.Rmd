...
```{r data processing,echo=TRUE,warning=FALSE,message=FALSE}
...
```{r densityanimals, echo=TRUE,eval=TRUE, include = TRUE}
...
```{r boxplotanimals, echo=TRUE,eval=TRUE, include = TRUE}
...
```{r barchartanimals, echo=TRUE,eval=TRUE, include = TRUE}
...
```{r intake, echo=TRUE,eval=TRUE, include = FALSE}
...
AIC(mod.loglinear)
```
We fond that collinearity exists between different animal types. We then check the data grouped by animal type and fond that there were only 2 rows of data for \textbf{BIRD}. Since we know that small size of data would cause collinearity, we decided to include the data for BIRD into WILDLIFE.
